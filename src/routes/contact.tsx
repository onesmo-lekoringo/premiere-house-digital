...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Stay in the Loop — Aurelia" },
      {
        name: "description",
        content:
          "Premiere invitations, teaser drops and opening-night dates from Aurelia — delivered before the carpet is rolled out.",
      },
      { property: "og:title", content: "Stay in the Loop — Aurelia" },
      {
        property: "og:description",
        content: "Join the Aurelia list for premiere invitations and reveals.",
      },
    ],
  }),
...
function ContactPage() {
  ...
  const mutation = useMutation<SignupRecord, Error, z.infer<typeof schema>>({
          ...
          {submitted ? (
            <div className="grain flex min-h-[24rem] flex-col items-start justify-center border border-primary bg-card p-10">
              ...
              <p className="mt-5 font-serif text-lg text-muted-foreground">
                Thank you, {mutation.data?.name.split(" ")[0]}. A confirmation is
                on its way to {mutation.data?.email}.
              </p>
              <button
            ...
            <form onSubmit={onSubmit} noValidate className="space-y-8">
              ...
              >
                {mutation.isPending ? "Sending…" : "Join the list ↗"}
              </button>
            </form>
          )}
        </Reveal>
